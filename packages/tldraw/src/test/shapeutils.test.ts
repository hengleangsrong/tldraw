...
import { FrameShape } from '../lib/shapes/frame/FrameShape'
import { GeoShape } from '../lib/shapes/geo/GeoShape'
...
describe('When interacting with a shape...', () => {
	it('fires rotate events', () => {
		...
		const util = editor.getShapeUtil(FrameShape)
	...
	it('cleans up events', () => {
		const util = editor.getShapeUtil(GeoShape)
